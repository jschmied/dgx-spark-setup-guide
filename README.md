...
| G | [Runbook: Qwen3.8-27B-NVFP4 + MTP on vLLM 0.27.1](appendix/qwen38-27b-nvfp4-vllm027.md) | Day-one deploy of **Qwen3.8-27B-NVFP4 on stock `pip install vllm==0.27.1`** (GB10) — the claim that GB10 needs a custom image/nightly is **false**: the native `FlashInferCutlassNvFp4LinearKernel` loads on the PyPI wheel. Headline is a **benchmark trap**: under spec-decode vLLM packs several accepted tokens into ONE SSE chunk, so delta-counting probes measure *deltas/s* and make MTP look like a **slowdown** (our first sweep "showed" 11.4→7.6 tok/s — an artifact). Corrected: **MTP n=3, ~2× short-prompt / ~1.5× on real work** — and the depth optimum **shifts down as context grows** (acceptance length 3.76 short → 2.84 at 4k), so short-prompt sweeps over-recommend depth. MTP wins at **every** concurrency level (+55–68% aggregate). Also: **`xhigh` (the vendor default) never converges** — 26k tokens of thinking, zero content, which scores as a false FAIL; ship `reasoning_effort=medium`. Plus the torch-2.13.0+cu130 pin, `qwen3_xml`≡`qwen3_coder`, the `reasoning_content`→`reasoning` rename, `minimal`/`max` returning HTTP 400, and **§9: cgroup limits CANNOT cap vLLM** (GPU allocs are invisible to cgroups — this config powercycled the box; protect `user.slice`, not just `ssh.service`) |
