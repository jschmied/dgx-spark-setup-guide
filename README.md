...
The reference workload is **`unsloth/Qwen3-Coder-Next-GGUF`** served with **`llama.cpp` / `llama-server`**, but most of the material applies to any GGUF model on this hardware.

## What you get if you follow the whole guide

- A non-root Linux service running `llama-server` with the model fully on the GB10 GPU
...
| 6 | [Run as systemd service](06-systemd-service.md) | env file, baseline unit, hardening flags |
...
| `MODEL_NAME` | Short directory name for the model (e.g. `qwen3-coder-next`) |
