...
| B | [DeepSeek-V4-Flash-180B & A4Q](appendix/deepseek-v4-flash-180b-and-a4q.md) | Bare-metal vLLM forks on GB10: fitting/building the REAP-pruned 180B (5 extraction gotchas + the unified-memory build-OOM lesson) — then **decommissioning** it (underperformed) and **productionizing A4Q** for the qwen36 NVFP4 fleet as `llm-switch a4q`. The tuning campaign: nvf4 KV (halves KV, non-compounding quality hit), the sm121 MoE-backend map (marlin vs the b12x single-stream cliff), and **CUDA graphs recovering decode 27→73 tok/s** for a recommended big-prompt agentic config (A4Q + graphs + nvf4 KV + 256k + marlin + MTP-off) |
