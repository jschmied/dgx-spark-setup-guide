...
| Models | `qwen3-coder-next`, `qwen36-35b-a3b` (local, served by the router); **Sonnet 4.6 †** added later |
| Samples | 1 per model per task (see caveats) |

> **† Sonnet 4.6 methodology caveat.** Sonnet was run as a **one-shot generator** (no tools, no iteration — `tool_uses: 0`) but through the **Claude Code agent harness** (its own system prompt) at **default sampling**, whereas the two local models got a bare chat completion at `temperature 0.2`. The task, extraction, and objective verdict are identical, so treat the cross-vendor comparison as **indicative, not controlled**.

**Headline:** all three get the *business logic* right; they differ on whether the **delivered artifact** builds and passes. `qwen36-35b-a3b` and `Sonnet 4.6` ship building, self-consistent projects; `qwen3-coder-next` has sound production logic but ships a broken artifact in both tasks (Go: `Close` panic; Java: test sources don't compile). `qwen3-coder-next` is far the cheapest/fastest.

## 13.2 Task A — Go cache: scorecard

| Check | qwen3-coder-next | qwen36-35b-a3b | Sonnet 4.6 † |
|---|---|---|---|
| Compiles + `go vet` clean | ✅ | ✅ | ✅ |
| **Neutral suite** `go test -race` | ❌ **panics on test 1** | ✅ **all pass** | ✅ **all pass** |
| Own suite `go test -race` | ❌ FAIL (1 panic + 2 assertions) | ✅ PASS | ⚠️ **16/17** (1 timing test) |
| `Close()` idempotent (rule 5) | ❌ `close of closed channel` | ✅ closed-channel guard | ✅ **`sync.Once`** (+ concurrent-close test) |
...
| Assumptions + complexity deliverable | ❌ omitted | ✅ thorough, documented | ✅ thorough, documented |
| Output | 4,011 tokens, no reasoning, fast | 14,315 tokens (mostly reasoning), 355 s | ~14,600 tokens |
| Test suite | larger (327 lines) but ≥2 **wrong** assertions | 159 lines, correct | 17 tests, 1 self-inconsistent (timing) |

**Winner: `qwen36-35b-a3b`** (only one whose own suite is fully green). `Sonnet 4.6` is logically correct with the most robust `Close`, but its delivered `go test` is red on one self-inconsistent timing test (§13.5). `qwen3-coder-next` wins on speed/cost.
...
**Sonnet 4.6 †** — the cleanest `Close` of the three (`sync.Once`, plus it wrote its own concurrent-close test); correct `container/list` LRU, correct expiry/LRU/stats; passes the neutral suite race-clean and documents `capacity==0 = unbounded`. Its **one own-suite failure is self-inflicted**: it clamps the janitor interval to a 1 s minimum, but `TestJanitorRemovesExpired` only waits 200 ms — so its own test and its own impl disagree on timing (the janitor logic itself is fine; entries also expire lazily). Net: production-correct, but the delivered `go test` is red on that one timing test — a milder version of the same "self-inconsistent test" failure mode coder-next showed.

## 13.6 Task B — Spring stack: scorecard

| Check | qwen3-coder-next | qwen36-35b-a3b | Sonnet 4.6 † |
|---|---|---|---|
| Deliverable `mvn test` | ❌ **does not compile** (test sources) | ✅ **BUILD SUCCESS — 7/7** | ✅ **BUILD SUCCESS — 12/12** |
| — Mockito unit tests | — (compile fail) | ✅ 5/5 | ✅ 8/8 |
| — `@DataJpaTest` (real Hibernate+H2) | — | ✅ 2/2 (`@Version` increment) | ✅ 4/4 (`@Version` increment **+ =0 on insert**) |
| **Neutral** Mockito suite vs *production* code | ✅ **5/5** | ✅ **5/5** | ✅ **5/5** |
| Self-consistent (entity API matches its own tests) | ❌ test calls `Account.setId()` it never defined | ✅ `setId` + `getVersion` present | ✅ sets id via reflection (clean entity) |
| Mockito strict-stub discipline | n/a (didn't compile) | ✅ documented + correct | ✅ correct (AssertJ + ArgumentCaptor) |
| Spec adherence | Spring Boot 3.3.2, skipped file labels | Spring Boot 3.3.5, all files + assumptions | Spring Boot 3.3.4, all files + assumptions |
| Output | 3,034 tokens, no reasoning, 86 s | 8,408 tokens (reasoning), 199 s | ~13,500 tokens |

**Winner: `Sonnet 4.6` / `qwen36-35b-a3b`** — both build and pass; Sonnet's suite is the most thorough (12 tests, incl. version-on-insert, null-amount, both unknown-account directions). `qwen3-coder-next` is the only one that doesn't compile.
...
Same shape as Task A: coder-next is fast with correct core logic but ships a non-compiling/self-inconsistent test artifact; qwen36 and Sonnet reason through it and ship green builds, at ~3–4× the tokens.
...
This mirrors the throughput data on [page 8](08-performance-tuning.md): qwen36 is faster per token but spends far more tokens (reasoning); coder-next is the cheaper path when its answer is correct — and a quick `go test`/`mvn test` catches the cases where it isn't. Reserve `Sonnet 4.6` for when output quality matters more than keeping inference on-box.

## 13.9 Caveats

- **Single sample per model per task** at `temperature 0.2` — indicative, not statistical. A firm ranking needs 3–5 samples each (page 12 §12.9).
- Two tasks already reduce single-task bias, and they **agree** here — but both reward careful lifecycle/edge-case reasoning, which favours the reasoning model. A different shape of task (algorithmic puzzle, large refactor, debugging) could shift the result.
- Cross-task consistency is the strongest signal in this evaluation: the *same* failure mode (correct logic, broken delivered tests) showed up for coder-next in both Go and Java — and, more mildly, for Sonnet on Go.
