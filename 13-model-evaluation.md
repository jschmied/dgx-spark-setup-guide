...
| Samples | **N=4** for all five local models (the dense `qwen36-27b` is the slow one, ~16–26k tokens/sample). **Sonnet 4.6 †** is a single indicative sample (different harness, not re-run). |
...
1. **Spring/Java production logic was correct for nearly every model on every sample** (neutral 4/4 across the board; the lone exception is the dense `qwen36-27b` at 3/4). The main differentiator is whether each model's *own delivered tests* compile and pass.
...
| `qwen36-27b` (1.0, top-p 0.95) | 1/4 | 1/4 | dense, slow + noisy: a compile error (`undefined: e`), a runtime panic under the neutral suite, and one non-delivery (hit the 26k-token cap mid-reasoning, no code) |
...
| `qwen36-27b` (1.0, top-p 0.95) | 3/4 | 2/4 | production logic correct on 3/4; the one miss is the sample whose Go run also overran (reasoning model, 26k-token non-delivery) |
...
- **Dense base Qwen3.6-27B → `qwen36-27b`.** Slow (dense, ~20 t/s with MTP) and the weakest local coder here — Go **1/4** (a compile error, a runtime panic, and one 26k-token non-delivery) and Java **3/4**. Run it for the base model's behaviour/style, not for coding throughput or reliability; gate every deliverable. MTP-accelerated (page 8 §8.8).
