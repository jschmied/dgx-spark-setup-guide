...
arm(){
  ...
  SUDO cp "$2" $SP/v1/core/sched/scheduler.py
  SUDO cp $S/coord.instr.py $SP/v1/core/kv_cache_coordinator.py
  SUDO cp $S/mgr.instr.py   $SP/v1/core/kv_cache_manager.py
  ...
  if [ $ok = 1 ]; then
    T0=$(date --iso-8601=seconds); sleep 1
    LLM_KEY="$LLM_KEY" python3 $S/trace.py 2>&1 | tail -4
    echo "  --- Spur ---"
    SUDO journalctl -u arm --no-pager -o cat --since "$T0" \
      | grep -oE "(HITPROBE|STOREPROBE|SCHEDPROBE)\[?[A-Z0-9]*\]? ?[^\"]{0,110}" | sed 's/^/    /'
  else echo "  KAM NICHT HOCH"; fi
...
arm "BASE" $S/sched.A_base.py
arm "PR"   $S/sched.B_pr.py
...
for i in $(seq 1 90); do
  [ "$(curl -sS -m 5 -o /dev/null -w '%{http_code}' http://127.0.0.1:8080/health 2>/dev/null)" = "200" ] && { echo "PROD WIEDER OBEN"; break; }; sleep 10; done
echo "ALL DONE"
