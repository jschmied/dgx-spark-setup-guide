...
At `temp 0.2`, **`gemma-4-26B-A4B` falls into verbatim repetition loops** and emits *no code at all*. The first write-up read that as "Gemma can't finish the Go task." It can — at the temperature its model card recommends (`temp ≈ 1.0`) it delivers production-correct code. A fixed low temperature is not neutral: it silently breaks any model whose architecture expects a higher operating point.
...
While grading Task B, `qwen36-35b-a3b` scored **0/3** on the neutral Java suite — alarming for a model that ships clean Spring builds. The cause was **my suite, not the model**: the neutral `NeutralTransferTest` hardcoded `new Account(owner, balance)`, but qwen36 (among others) legitimately defined `Account(Long id, String owner, BigDecimal balance)`. The suite *failed to compile*, which looked like a logic failure but was a measurement artifact.

**Fix:** the neutral `acc()` helper now builds `Account` by **reflection** — it tries `(String,BigDecimal)`, then `(Long,String,BigDecimal)`, then no-arg + setters — mirroring the trick Sonnet used in its own test. With that fix, **every model's Spring production logic passes** (neutral 4/4 across the board). The earlier qwen36 "0/3" Java failure vanished entirely. The general lesson: a neutral suite must test *logic*, not assume a specific surface the model was never told to expose.
...
That is why Gemma's deployed sampling is **`temp 1.0 / top-k 64 / min-p 0.1`, top-p disabled** (page 8 §8.10).

## 14.5 Does the min-p cure generalise?

**It's model-specific — sweep each model from scratch.** min-p clearly fixed Gemma 26B's Go variance, but it is *not* a universal cure for recommended-temp Go noise: some models need min-p paired with a *lower* temperature, others loop at low temp and want the plain high-temp default, and the right operating point does **not** follow from the base model or architecture. Even within the Gemma family the point differs — the larger, reasoning-variant **`gemma-4-31B`** is production-correct on Go **3/4** at the plain `temp 1.0 / top-p 0.95 / top-k 64` and was never tuned with min-p (its one Go miss is a trivial unused-variable compile error, not tail-sampling noise). So treat each new model as its own sweep, and — whatever the sampling — **gate every deliverable on `go build` / `mvn test`**, because the production logic is usually right while the *delivered tests* are where models break.
...
- **Single-shot sampling sweep** — `./sweep-java.sh <model>` runs one Java shot per config across a temp/top-k/min-p grid (how the Gemma sweep in §14.4 was mapped).
...
2. If it loops at the recommended temperature, that model needs `temp ≈ 1.0` (e.g. Gemma).
3. If it delivers but with a *different bug each run*, it's tail-sampling noise — try **min-p 0.1 with top-p off**, and if that isn't enough, **sweep temperature** (lower is usually better for deterministic code; some models need both min-p 0.1 *and* a lower temp). Use min-p 0.1, not 0.05 — a looser tail still leaks bugs.
